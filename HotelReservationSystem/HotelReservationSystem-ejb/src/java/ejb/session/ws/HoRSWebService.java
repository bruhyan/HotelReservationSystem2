...
public class HoRSWebService {
    ...
    public BigDecimal calculateTotalPrice(@WebParam(name = "roomTypes") List<RoomTypeEntity> roomTypes, @WebParam(name = "nights") int nights, @WebParam(name = "currentDate") Date currentDay) {
        BigDecimal totalPrice = new BigDecimal(0.00);

        for (int i = 0; i < nights; i++) {
            for (RoomTypeEntity roomType : roomTypes) {
                try {
                    RoomRatesEntity prevailingRate = roomTypeControllerLocal.findOnlineRateForRoomType(roomType.getRoomTypeId(), currentDay);
